...
<div class="container">
  ...
  @if(count($select) > 0)
    ...
    <td scope="row" data-label="رقم الطلب">{{ $selected->id }}</td>
      ...
      <td data-label="">
      <a class="box-linkk" href="#ModalShowSystem" data-toggle="modal" data-target-orderid="{{ $selected->id }}" data-target-sendtext="{{ $selected->sendtext }}" data-target-sendfile="{{ $selected->sendfile }}" data-target-recivetext="{{ $selected->recivetext }}" data-target-recivefile="{{ $selected->recivefile }}" data-target="#ModalShowSystem">معالجة ... <i class="fas fa-arrow-circle-left"></i></a>
      </td>
    </tr>
    @endforeach
    @endif
...
<hr>
        ...
        <div class="modal-footer">
          ...
          <form action="/OrderProccessWite" method="POST" role="form">
            {{ csrf_field() }}
          <input type="hidden" name="orderid" id="orderid" class="form-control" value="">
          <button type="submit" class="box-linkk">معالجة الطلب</button>
          <button type="button" class="box-linkkkk" data-dismiss="modal">إغلاق</button>
          </form>
